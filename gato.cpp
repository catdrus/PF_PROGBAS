...
struct AudioManager {
    ...
    void playClick() {
        if (clickBuffer.getDuration().asSeconds() > 0 && !sfxMuted) {
            clickSound.play();
        }
    }
    
    void playMove() {
        if (moveBuffer.getDuration().asSeconds() > 0 && !sfxMuted) {
            moveSound.play();
        }
    }
    
    void playWin() {
        if (winBuffer.getDuration().asSeconds() > 0 && !sfxMuted) {
            winSound.play();
        }
    }
    
    void playDraw() {
        if (drawBuffer.getDuration().asSeconds() > 0 && !sfxMuted) {
            drawSound.play();
        }
    }
    
    void playHover() {
        if (hoverBuffer.getDuration().asSeconds() > 0 && hoverSound.getStatus() != sf::Sound::Playing && !sfxMuted) {
            hoverSound.play();
        }
    }
    
    void playMusic() {
        if (bgMusic.getDuration().asSeconds() > 0 && !musicMuted) {
            bgMusic.play();
        }
    }
...
void drawMenu(sf::RenderWindow& window, const sf::Font& font) {
    ...
    playText.setPosition(
        ...
        280.f + (80.f - textBounds.height) / 2.f - textBounds.top
    );
    window.draw(playText);

    // Botón CONFIGURACIÓN con hover
    bool isHoveringSettings = (mousePos.x >= 200 && mousePos.x <= 500 && 
                               mousePos.y >= 380 && mousePos.y <= 460);

    // Sombra del botón CONFIGURACIÓN
    ...
    settingsText.setPosition(
        200.f + (300.f - textBounds.width) / 2.f - textBounds.left,
        380.f + (80.f - textBounds.height) / 2.f - textBounds.top
    );
...
void drawSettings(sf::RenderWindow& window, const sf::Font& font) {
    ...
    title.setPosition((WINDOW_SIZE - titleBounds.width) / 2.f, 50.f);
    window.draw(title);
    ...
    musicMuteText.setFont(font);
    musicMuteText.setString(audio.musicMuted ? "ON" : "OFF");
    ...
    musicMuteText.setPosition(
        500.f + (100.f - muteBounds.width) / 2.f - muteBounds.left,
        190.f + (40.f - muteBounds.height) / 2.f - muteBounds.top
    );
    ...
    sf::Text sfxMuteText;
    sfxMuteText.setFont(font);
    sfxMuteText.setString(audio.sfxMuted ? "ON" : "OFF");
    ...
    sfxMuteText.setPosition(
        500.f + (100.f - sfxMuteBounds.width) / 2.f - sfxMuteBounds.left,
        320.f + (40.f - sfxMuteBounds.height) / 2.f - sfxMuteBounds.top
    );
    ...
    backText.setPosition(
        250.f + (200.f - backBounds.width) / 2.f - backBounds.left,
        450.f + (70.f - backBounds.height) / 2.f - backBounds.top
    );
    ...
    instructions.setFillColor(sf::Color(100, 100, 100));
    sf::FloatRect instBounds = instructions.getLocalBounds();
    instructions.setPosition((WINDOW_SIZE - instBounds.width) / 2.f, 580.f);
    window.draw(instructions);
}
    ...
    while (window.isOpen()) {
            ...
            if (event.type == sf::Event::MouseButtonPressed) {
                if (event.mouseButton.button == sf::Mouse::Left) {
                    ...
                    if (currentState == GameState::Game && !gameOver) {
                        ...
                        // Click en barra de música
                        else if (mx >= 100 && mx <= 400 && my >= 200 && my <= 220) {
                            isDraggingMusic = true;
                        }
                        // Click en barra de efectos
        ...
        if (currentState == GameState::Settings) {
            ...
            if (isDraggingMusic) {
                ...
                if (newVolume > 100.f) newVolume = 100.f;
